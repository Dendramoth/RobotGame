...
public class CliffVerticalReversed extends GameStaticObject {
    ...
    public CliffVerticalReversed(Point possition, double width, double heigh, GraphicsContext graphicsContext, MonitorWindow monitorWindow) {
        super(getPoints(possition), getPoints(possition), getPoints(possition), possition, width, heigh, 0, graphicsContext, monitorWindow, LoadAllResources.getMapOfAllImages().get("cliffVerticalReversed"));
    }

    private static List<Point> getPoints(Point possition) {
        List<Point> pointList = new ArrayList<>();
        pointList.add(new Point(116 + possition.getCoordX(), 0 + possition.getCoordY()));
        pointList.add(new Point(116 + possition.getCoordX(), 1024 + possition.getCoordY()));
        pointList.add(new Point(120 + possition.getCoordX(), 1024 + possition.getCoordY()));
        pointList.add(new Point(120 + possition.getCoordX(), 0 + possition.getCoordY()));
        return pointList;
    }
